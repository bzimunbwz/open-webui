...
<div class="flex flex-col h-full overflow-y-auto">
	<div class="px-6 pt-5 pb-3">
		<div class="flex items-center justify-between mb-1">
			...
			<div class="flex gap-2">
				...
				<button on:click={startNew} class="text-xs px-3 py-1.5 bg-blue-600 text-gray-900 dark:text-white rounded-lg hover:bg-blue-700 transition">+ New Package</button>
			</div>
		</div>
	...
	{#if showForm}
		<div class="mx-6 mb-4 bg-white dark:bg-gray-900 rounded-xl p-5 border border-gray-200 dark:border-gray-800">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
				<div>
					<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Package ID</label>
					<input bind:value={form.id} placeholder="e.g. pro" disabled={!!editingPkg}
						class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-3 py-2 text-sm {editingPkg ? 'opacity-50' : ''}" />
				</div>
				<div>
				...
				<div>
					<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Tier Level</label>
					<select bind:value={form.tier} class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-3 py-2 text-sm">
				...
				<div>
					<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Sort Order</label>
			...
			<div class="mb-3">
				<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Description</label>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
				<div>
					<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Monthly Price (USDT)</label>
					<input type="number" step="0.01" bind:value={form.price_monthly}
						class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-3 py-2 text-sm" />
				</div>
				<div>
					<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Yearly Price (USDT)</label>
			...
			<!-- Features -->
			<div class="mb-3">
				<label class="text-xs text-gray-600 dark:text-gray-400 mb-1 block">Features (shown on pricing page)</label>
				{#each form.features as feature, i}
					<div class="flex gap-2 mb-1">
						<input bind:value={form.features[i]} placeholder="Feature description"
							class="flex-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-3 py-2 text-sm" />
			...
			<!-- Model access -->
			<div class="mb-3">
				<label class="text-xs text-gray-600 dark:text-gray-400 mb-2 block">Model Access (leave empty = all paid models based on tier)</label>
				<div class="flex flex-wrap gap-2">
					{#each facadeModels as model}
						<button
							on:click={() => toggleModel(model.id)}
							class="text-xs px-3 py-1.5 rounded-lg border transition {form.models.includes(model.id) ? 'bg-blue-600 border-blue-500 text-gray-900 dark:text-white' : 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-gray-600'}"
						>{model.name}</button>
					{/each}
				</div>
			</div>
			...
			<div class="flex items-center gap-2 mb-4">
				...
				<span class="text-sm text-gray-600 dark:text-gray-400">{form.active ? 'Active' : 'Inactive'}</span>
			</div>

			<div class="flex gap-2">
				<button on:click={save} class="px-4 py-2 bg-blue-600 text-gray-900 dark:text-white text-sm rounded-lg hover:bg-blue-700 transition">{editingPkg ? 'Save' : 'Create'}</button>
				<button on:click={() => { showForm = false; editingPkg = null; }} class="px-4 py-2 bg-gray-100 dark:bg-gray-800 text-sm rounded-lg hover:bg-gray-700 transition">Cancel</button>
			</div>
		</div>
	{/if}
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 px-6 pb-6">
		{#each packages as pkg (pkg.id)}
			<div class="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 p-5 flex flex-col {!pkg.active ? 'opacity-50' : ''}">
				...
				{#if pkg.features?.length}
					<ul class="text-xs text-gray-600 dark:text-gray-400 mb-3 space-y-1 flex-1">
