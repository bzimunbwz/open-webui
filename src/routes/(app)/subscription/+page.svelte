<script lang="ts">
	...
	async function selectPackage(pkgId: string) {
		selectedPkg = pkgId;
		showPayment = true;
		paymentMethod = '';
		couponCode = '';
		txHash = '';
		// Land at the top of the checkout step, not wherever the plan grid was scrolled to.
		await tick();
...
<div class="flex flex-col h-full w-full flex-1 overflow-y-auto" bind:this={scrollEl}>
	<div class="max-w-4xl lg:max-w-5xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8">
		...
		{:else if showPayment && selectedPkgData}
			<!-- ════════ STEP 2 — Checkout ════════ -->
			<div in:fly={{ x: 24, duration: 200 }} class="max-w-lg mx-auto">
				<button
					on:click={backToPlans}
					class="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-300 transition mb-5"
				>
				...
				<!-- Order summary -->
				<div class="bg-gray-900/50 rounded-2xl border border-gray-800 p-5 mb-4">
					<div class="flex items-start justify-between gap-3">
						<div>
							<div class="text-[11px] uppercase font-bold text-orange-400 tracking-wide">You're subscribing to</div>
							<h3 class="font-bold text-xl mt-1">{selectedPkgData.name}</h3>
							<p class="text-xs text-gray-500 mt-1">{duration === 'yearly' ? 'Yearly' : 'Monthly'} billing</p>
						</div>
